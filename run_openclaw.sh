#!/bin/bash
# 赛博印钞机 Pro - 核心胶水脚本（openclaw路径自动检测版）
...
if [ -z "$CLAW_BIN" ]; then
  if command -v openclaw &> /dev/null; then
    CLAW_BIN="$(command -v openclaw)"
  fi
fi
# 最终校验
if [ -z "$CLAW_BIN" ] || [ ! -f "$CLAW_BIN" ]; then
  echo "❌ 致命错误：未找到openclaw命令！"
  echo "💡 请确认OpenClaw/Claude Code已正确安装，或设置环境变量OPENCLAW_CLAW_PATH指定openclaw路径"
  exit 1
fi
chmod +x "$CLAW_BIN" 2>/dev/null
echo "✅ 使用openclaw命令：$CLAW_BIN"
...
if [ "$ENABLE_MULTI_AGENT" = "true" ]; then
  ...
  safe_openclaw_chat "$OUTLINE_PROMPT" 60 "/tmp/chapter_outline.txt"
  if [ $? -ne 0 ]; then
  ...
  if [ "$ENABLE_MULTI_AGENT" = "true" ]; then
    ...
    safe_openclaw_chat "$CONTENT_PROMPT" 240 "/tmp/chapter_content.txt"
    if [ $? -ne 0 ]; then
        echo "❌ 正文生成失败"
        exit 1
    fi
  ...
  if [ "$ENABLE_MULTI_AGENT" = "true" ]; then
    ...
    safe_openclaw_chat "$CHECK_PROMPT" 120 "/tmp/chapter_final.txt"
    if [ $? -ne 0 ]; then
...
if [ "$ENABLE_MULTI_AGENT" = "false" ] || [ -z "$FINAL_CONTENT" ]; then
  ...
  safe_openclaw_chat "$SINGLE_PROMPT" 300 "/tmp/chapter_final.txt"
  if [ $? -ne 0 ]; then
    echo "❌ 正文生成失败"
    exit 1
  fi
...
if [ "$ENABLE_HUMANIZER" = "true" ]; then
  ...
  safe_openclaw_chat "$HUMANIZE_PROMPT" 180 "/tmp/chapter_humanized.txt"
  if [ $? -eq 0 ]; then
