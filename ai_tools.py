...
def parse_tool_call(text: str) -> Optional[Tuple[str, Dict[str, Any]]]:
    ...
    支持的格式:
    - <|tool_call|>{"name": "tool_name", "parameters": {...}}<|end_of_tool_call|>
    - 或简单的自然语言描述
...
def execute_tool(tool_name: str, parameters: Dict[str, Any]) -> str:
    ...
    if tool.requires_unlimited and not is_unlimited_mode():
        return f"错误: 工具 '{tool_name}' 仅在全能模式下可用"
